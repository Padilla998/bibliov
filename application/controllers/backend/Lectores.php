...
class Lectores extends CI_Controller
{
    ...
    public function store()
    {
        ...
        $this->form_validation->set_rules('dui', 'DUI', 'trim|required|exact_length[9]|is_unique[lectores.dui]', array('required' => 'Debes proporcionar un %s.', 'is_unique' => 'Este %s ya existe', 'exact_length' => 'El %s debe ser 9 digitos'));
    ...
    public function update()
    {
        ...
        if ($dui != $lector_actual->dui) {
            $is_unique = '|is_unique[lectores.dui]';
        } else {
            $is_unique = '';
        }
        $this->form_validation->set_rules('nombres', 'Nombres', 'trim|required', array('required' => 'Debes proporcionar un %s.'));
        $this->form_validation->set_rules('apellidos', 'Apellidos', 'trim|required', array('required' => 'Debes proporcionar un %s.'));
        $this->form_validation->set_rules('dui', 'DUI', 'trim|required|exact_length[9]' . $is_unique, array('required' => 'Debes proporcionar un %s.', 'is_unique' => 'Este %s ya existe', 'exact_length' => 'El %s debe ser 9 digitos'));
        $this->form_validation->set_rules('direccion', 'Direccion', 'trim|required', array('required' => 'Debes proporcionar un %s.'));
        $this->form_validation->set_rules('idtipolector', 'Tipo Lector', 'trim|required', array('required' => 'Debes proporcionar un %s.'));
        $this->form_validation->set_error_delimiters('<span class="help-block">', '</span>');
    ...
    public function comprobardui()
    {
        $dui = $this->input->post("dui");
        $res = $this->Lectores_model->comprobardui($dui);
        if ($res) {
            if ($res->estado == 1) {
